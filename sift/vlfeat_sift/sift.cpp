...
main(int argc, char **argv)
{
    ...
    * --------------------------------------------------------------- */
    ...
    while (argc--) {
        ...
        gray = cv::imread(name, CV_LOAD_IMAGE_GRAYSCALE);
        if (!gray.data) {
            cerr << "No image data" << endl;
            exit(-1);
        }

        fdata = new float[gray.rows * gray.cols];
        for (int i = 0; i < gray.rows; ++i) {
            for (int j = 0; j < gray.cols; ++j) {
                fdata[j + gray.cols*i] = gray.at<unsigned char>(i, j);
            }
        }
        ...
        filt = vl_sift_new (gray.rows, gray.cols, O, S, omin) ;
